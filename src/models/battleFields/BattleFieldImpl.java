...
public class BattleFieldImpl implements Battlefield {
    ...
    private static final int CARD_DAMAGE_POINTS_INCREASE = 300;
    ...
    private void processFight(Player attackPlayer, Player enemyPlayer) {
        while (true) {
            int attackPlayerDamagePoint = attackPlayer.getCardRepository()
                    .getCards()
                    .stream()
                    .mapToInt(Card::getDamagePoints)
                    .sum();
            enemyPlayer.takeDamage(attackPlayerDamagePoint);

            if (enemyPlayer.isDead()) {
                return;
            }
            int enemyPlayerDamagePoint = enemyPlayer.getCardRepository()
                    .getCards()
                    .stream()
                    .mapToInt(Card::getDamagePoints)
                    .sum();
            attackPlayer.takeDamage(enemyPlayerDamagePoint);
    ...
    private void preFightPreparation(Player player) {
        ...
        player.getCardRepository().getCards().stream().forEach(c->
                c.setDamagePoints(c.getDamagePoints() + CARD_DAMAGE_POINTS_INCREASE));
    }
}
